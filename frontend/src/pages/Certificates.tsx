  ...
  const fetchCerts = async () => {
    ...
    try {
      const data = await api.certificates.list({
        ...
        type: certType !== 'all' && statusFilter === '' ? undefined : undefined
      });
      setCerts(data);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 relative z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {[
          { title: "Barangay Clearance", cost: "₱150.00", color: "border-l-gov-blue-500", desc: "For employment, local licensing, background check verification." },
          { title: "Indigency Certificate", cost: "Free", color: "border-l-emerald-500", desc: "For medical assistance, scholarships, social welfare requests." },
          { title: "Residency Certificate", cost: "₱100.00", color: "border-l-gov-gold-500", desc: "Proof of residence verification, bank accounts setup." },
          { title: "Business Clearance", cost: "₱300.00", color: "border-l-indigo-500", desc: "Required for municipal business permit operation." },
          { title: "Cedula (CTC)", cost: "₱50.00", color: "border-l-rose-500", desc: "Official proof of community identity and taxes paid." }
        ].map((item, idx) => (
          <div key={idx} className={`bg-white dark:bg-slate-900 p-4 rounded-2xl border-l-4 ${item.color} border border-slate-200/50 dark:border-slate-800/50 shadow-sm flex flex-col justify-between`}>
            ...
            <span className="text-xs font-black text-slate-700 dark:text-slate-300 mt-3 block">{item.cost}</span>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/50 dark:border-slate-800/50 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="p-10 text-center text-xs font-bold text-slate-500 dark:text-slate-400">Loading certificate logs...</div>
          ) : filteredCerts.length === 0 ? (
            <div className="p-10 text-center text-xs font-bold text-slate-500 dark:text-slate-400">No certificate requests found matching criteria.</div>
          ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs">
                {filteredCerts.map((cert) => (
                  <tr key={cert.id} className="hover:bg-slate-50/30 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="p-4 text-slate-500 dark:text-slate-400 truncate max-w-[180px]">{cert.purpose}</td>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/50 rounded-3xl max-w-md w-full shadow-2xl p-6 glass-panel relative overflow-hidden animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 block mb-1">Certificate Type</label>
                <select
                  value={certType}
              ...
              <div>
                <label className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400 block mb-1">Purpose / Reason</label>
                <textarea
                  value={purpose}
